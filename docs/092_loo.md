...
(\#eq:kl-div-cont-t2)
\end{equation}
Anche nella \@ref(eq:kl-mod-comp), però, abbiamo una quantità sconosciuta, ovvero $p_t(y)$. Quindi, in questa forma, la \@ref(eq:kl-div-cont-t2) non può essere usata. Riscriviamo dunque la \@ref(eq:kl-div-cont-t2) facendo riferimento alla distribuzione predittiva a posteriori, $p(\tilde{y} \mid y)$, perché ciò a cui siamo interessati è la divergenza di $p(\tilde{y} \mid y)$ da $p_{t}(y)$:
\begin{equation}
