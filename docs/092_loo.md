...
Ma come è possibile valutare l'accuratezza (predittiva) di un modello? Per rispondere a questa domanda @McElreath_rethinking ci fa notare che è necessario evitare due opposti errori:

- il _sovra-adattamento_ (_overfitting_) --- il modello non si generalizza bene a nuovi dati futuri perché fa leva informazioni irrilevanti presenti nel campione  esaminato; tali aspetti idiosincratici del campione necessariamente non si ritroveranno in altri campioni di dati;
...
In questo capitolo verranno presentati due metodi [detti _criterio di informazione_ (_information criteria_) e _validazione incrociata_ (_cross-validation_)] che, evitando il sovra- e il sotto-adattamento, consentono di valutare la capacità predittiva di un modello e di determinare quale, tra due o più modelli, sia quello da preferire.
...
La divergenza di Kullback-Liebler introduce un piccolo cambiamento alla \@ref(eq:entropy): anziché considerare una sola distribuzione di probabilità, $p_t$, considera anche una approssimazione a tale distribuzione, ovvero $p_{\mathcal{M}}$. Calcolando la differenza dei logaritmi dei valori delle due distribuzioni si giunge alla \@ref(eq:kldivergence).
...
#> $ mom_age   <dbl> 1.5602, 0.8198, 1.5602, 0.8198, 1.5602, -1.7718, -1.0313, 0.~
```
...
Calcoliamo ora la quantità $\elpd$ per i due modelli:
...
#> m2 -2.2       5.0
```
I risultati indicano che il Modello `m1` ha il valore $\elpd$ più basso (e quindi sarebbe quello da preferire). Tuttavia, se si confronta la differenza $\elpd$ tra il Modello `m1` e il Modello `m2` e si tiene in considerazione l'errore standard corrispondente (nella colonna `se_diff`), la differenza ottenuta risulta essere relativamente piccola. Per il Modello `m1` $\elpd$ è uguale a -567.2684 e per `m2` abbiamo -569.5050.  La differenza è (-567.2684 - -569.5050) pari a 2.24 con un errore standard stimato di 5.0. In altre parole, possiamo dire che i dati suggeriscono che la vera differenza in $\elpd$ tra `m1` e `m2` sia compresa tra $\pm$ 2 errori standard (ovvero nel caso presente, 10 unità) dalla differenza stimata di -2.2 unità, ovvero $-2.2 \pm 2 \cdot 5 = (-12.2, 7.8)$. Dato che la differenza $\elpd$ pari a zero è compresa entro l'intervallo di $\pm$ due standard error dalla differenza stimata, non c'è evidenza che l'accuratezza predittiva a posteriori di `m1` sia superiore a quella di `m2`. Inoltre, dato che il Modello `m2` è più semplice di `m1`, possiamo concludere che `m2` è il modello migliore tra i due considerati.
...
```
Considerando l'intervallo a posteriori del 95%, anche in questo caso concludiamo che non c'è evidenza che l'uso di un solo predittore faccia diminuire la capacità predittiva del modello.
