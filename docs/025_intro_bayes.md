...
<!-- - L'*inferenza statistica* è un tipo di inferenza induttiva che è specificamente formulata come un problema inverso. L'inferenza statistica è quell'insieme di procedure che hanno lo scopo di quantificare quanto più plausibile è A dopo aver osservato B. Per svolgere l'inferenza statistica, è necessario quantificare la plausibilità. La probabilità ci fornisce lo strumento per fare questo. -->

<!-- Quindi, l'inferenza statistica richiede una teoria della probabilità. La teoria della probabilità è una generalizzazione della logica. A causa di questa connessione logica e del suo ruolo cruciale nella scienza, E. T. Jaynes afferma che la probabilità è la ``logica della scienza''. -->
