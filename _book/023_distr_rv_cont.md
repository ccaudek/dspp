...
\begin{equation}
\text{Beta}(\pi \mid \alpha, \beta) = \frac{\Gamma(\alpha+b)}{\Gamma(\alpha)\Gamma(\beta)}\pi^{\alpha-1} (1-\pi)^{\beta-1} \quad \text{per } \theta \in [0, 1],
\end{equation}
laddove $\Gamma(\cdot)$ è la funzione Gamma. Per qualsiasi intero positivo, $\Gamma(n) = (n-1)!$.
:::

I termini $\alpha$ e $\beta$ sono i parametri della distribuzione e devono essere entrambi positivi.^[I parametri $\alpha$ e $\beta$ della distribuzione Beta possono essere interpretati come l'espressione delle nostre credenze a priori relativamente alla probabilità di successo: il parametro $\alpha$ rappresenta il numero di "successi" e il parametro $\beta$ il numero di "insuccessi".] Il rapporto $\frac{\Gamma(\alpha+b)}{\Gamma(\alpha)\Gamma(\beta)}$ è una costante di normalizzazione:
