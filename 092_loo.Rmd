...
\noindent
Confrontiamo $\elpd$ del modello completo e del modello semplificato:

```{r}
loo::loo_compare(loo1, loo2)
```
\noindent
I risultati indicano che il Modello `m1` ha il valore $\elpd$ più basso (e quindi sarebbe quello da preferire). Tuttavia, se si confronta la differenza in $\elpd$ tra il Modello `m1` e il Modello `m2`, e si tiene in considerazione l'errore standard corrispondente (nella colonna `se_diff`), la differenza ottenuta risulta essere relativamente piccola. Per il Modello `m1` $\elpd$ è uguale a -567.3 e per `m2` è -569.5.  La differenza è (-567.3 - -569.5) pari a 2.2 con un errore standard stimato di 5.0. I dati dunque suggeriscono che la vera differenza in $\elpd$ tra `m1` e `m2` sia compresa tra $\pm$ 2 errori standard (ovvero nel caso presente, 10 unità) dalla differenza stimata di -2.2 unità, ovvero sia inclusa nell'intervallo $-2.2 \pm 2 \cdot 5 = (-12.2, 7.8)$. Dato che una differenza $\elpd$ pari a zero è compresa nell'intervallo di $\pm$ due standard error dalla differenza stimata, non c'è evidenza che l'accuratezza predittiva a posteriori di `m1` sia superiore a quella di `m2`. Inoltre, dato che il Modello `m2` è più semplice di `m1`, possiamo concludere che `m2` è il modello migliore tra i due considerati.
