...
```{r rethinkingmodlearn, echo=FALSE, fig.cap="Come apprende un modello statistico. Ciascun lancio del mappamondo produce un'osservazione: acqua (A) o terra (T). La stima del modello statistico della proporzione di acqua sulla superficie terreste è espressa nei termini del grado di credibilità di ciascun possibile valore $\theta$ (proporzione di acqua). Le linee e le curve in questa figura rappresentano il grado di credibilità fornito dal modello. In ogni diagramma, le credibilità calcolate in base alle informazioni precedenti (curva tratteggiata) vengono aggiornate alla luce dell'ultima osservazione che è stata ottenuta per produrre un nuovo insieme di valori di credibilità (curva solida).", out.width = '100%'}
...
Dalla figura notiamo che, per questi dati, il massimo della funzione di log-verosimiglianza calcolata per via numerica è uguale a `r round(mean(d$y), 2)`. Tale valore è identico alla media dei dati campionari e corrisponde al risultato teorico della \@ref(eq:lldepression).
